...
var CleverTapAPI = function (CleverTapAccountId, CleverTapAccountPasscode, CleverTapRegion) {
    ...
    if(CleverTapRegion && !['in1', 'sg1', 'sk1', 'eu1'].includes(CleverTapRegion)) {
        throw new Error("Please provide a valid CleverTap Region");
    }

    var subDomain = null;

    if (CleverTapRegion && CleverTapRegion !== 'eu1') {
        subDomain = CleverTapRegion;
    }

    this.accountId = CleverTapAccountId;
    this.accountPasscode = CleverTapAccountPasscode;

    this.apiEndpoint = `https://${subDomain ? `${subDomain}.`: ''}${API_HOSTNAME}/${API_VERSION}`
